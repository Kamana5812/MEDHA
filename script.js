...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function initTestimonialSlider() {
    if (
      !DOM.testimonialsTrack ||
      !DOM.prevBtn ||
      !DOM.nextBtn ||
      !DOM.testimonialCards.length
    )
      return;
    ...
    const updateSlider = () => {
      ...
      const margin =
        parseFloat(
          window.getComputedStyle(DOM.testimonialCards[0]).marginRight,
        ) || 0;
  ...
  // ==========================================================================
  // 9. CONTACT FORM — client validation + API submission
  // ==========================================================================
  //
  // Client-side validation exists for speed of feedback, not for safety.
  // The server re-checks everything, and the server's verdict wins: if it
  // returns field errors, they are painted onto the form below.
  //
  function initContactForm() {
    ...
    const showError = (input, message) => {
      ...
      const msgEl = document.getElementById(`error-${input.id}`);
    ...
    /** Returns an errors object, empty when the form is clean. */
    const validateLocally = (values) => {
      ...
      fields.forEach((id) => {
        ...
        if (!value) {
          ...
          errors[id] = `${label(id)} must be at least ${rule.min} characters.`;
        } else if (value.length > rule.max) {
          errors[id] = `${label(id)} must be ${rule.max} characters or fewer.`;
        }
      });
    ...
    if (messageEl && counterEl) {
      const updateCount = () => {
        ...
        counterEl.innerText = `${len} / ${RULES.message.max}`;
        counterEl.className =
          "char-count" +
          (len >= RULES.message.max
            ? " at-limit"
            : len > RULES.message.max * 0.9
              ? " near-limit"
              : "");
      };
    ...
    // less punishing than leaving red text until the next submit.
    fields.forEach((id) => {
    ...
    DOM.contactForm.addEventListener("submit", async (e) => {
      ...
      if (Object.keys(localErrors).length > 0) {
        Object.entries(localErrors).forEach(([id, msg]) =>
          showError(inputs[id], msg),
        );
        inputs[Object.keys(localErrors)[0]].focus();
      ...
      // Don't leave the button stuck on "Sending..." forever if the
      // network hangs — abort after 20 seconds and let the user retry.
      ...
      try {
        ...
        // A 5xx from a proxy can be HTML, so parsing may fail.
        let payload = {};
        try {
          payload = await response.json();
        } catch (_) {
          payload = { message: "Unexpected response from the server." };
        }

        if (response.ok && payload.success) {
          DOM.contactForm.reset();
          if (counterEl) counterEl.innerText = `0 / ${RULES.message.max}`;
          DOM.submitBtn.innerText = "Message Sent!";
          DOM.submitBtn.style.background = "#22c55e";
          DOM.submitBtn.style.boxShadow = "none";
          setStatus(
            payload.message || "Thanks! We'll be in touch soon.",
            "success",
          );
        } else {
          // 422 carries per-field errors; paint them on the inputs.
          if (payload.errors && typeof payload.errors === "object") {
            Object.entries(payload.errors).forEach(([id, msg]) => {
              if (inputs[id]) showError(inputs[id], msg);
            });
          }
          setStatus(
        ...
        // Brief cooldown so the success state stays readable.
        setTimeout(() => {
          DOM.submitBtn.innerText = originalText;
  ...
  function initGSAPAnimations() {
    ...
    let ctx = gsap.context(() => {
      ...
      if (
        ...
        const xToOutline = gsap.quickTo(cursorOutline, "x", {
          duration: 0.25,
          ease: "power3.out",
        });
        const yToOutline = gsap.quickTo(cursorOutline, "y", {
          duration: 0.25,
          ease: "power3.out",
        });
        ...
        const interactiveElements = document.querySelectorAll(
          "a, button, .interactive, .stat-item, .program-card",
        );
      ...
      masterTl
        .from(DOM.navbar, { y: -50, opacity: 0, duration: 1.5 })
        .from(
          ".hero-content .badge",
        ...
        .from(
          ".hero-cta .btn",
          { y: 30, opacity: 0, stagger: 0.15, ease: "back.out(1.7)" },
          "-=1.6",
        )
        .from(
          ".hero-3d",
      ...
      const sections = [
        {
          ...
          anim: { y: 60, opacity: 0, stagger: 0.2 },
        },
        {
          trigger: ".about",
        ...
        },
        {
          ...
          anim: { y: 50, scale: 0.9, opacity: 0, stagger: 0.1 },
        },
        {
          trigger: "#mentors",
          targets: ".mentor-card",
        ...
        },
        {
          trigger: "#events",
          targets: ".event-card",
          anim: { x: -50, opacity: 0, stagger: 0.1 },
        },
        {
      ...
      const playgroundTl = gsap.timeline({
        scrollTrigger: { trigger: "#playground", start: "top 75%" },
      });
      playgroundTl
